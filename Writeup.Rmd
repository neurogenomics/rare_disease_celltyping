...
params:
  ...
  author: "Jai Chapman"
  date: "03/03/2021"
  #results_data: "CTD_DescartesHuman.rda"
  results_data: "Results_merged.rda"
  phenotype_to_genes_txt_file: "phenotype_to_genes.txt"
  # searching for enrichments in phenotypes related to key words
  keyword_1: "heart"
  ...
  # searching for enrichments in phenotypes under parent term
  parent_term_example: "Abnormality of the pancreas"
  parent_term_example2: "Recurrent infections"
  # determine what cells are enriched in a group of phenotypes (rather, ID surprising phenotypes for a given cell type)
  expected_terms: "Abnormality of the cardiovascular system"
  cell_of_interest: "Cardiac muscle cells"
  related_patterns: c("muscle","exercise","weakness","fatigue")
  ...
  # Add section - Significantly enriched phenotypes for given cell
---
...
library(scales)
genedata = read.delim(params$phenotype_to_genes_txt_file, skip =1) #HPO annotation
...
```{r, include = FALSE}
most_sig = ResultsSignif[order(ResultsSignif$fold_change, decreasing = TRUE),][1,]
```

The strongest enrichment is seen with `r most_sig$list[1]` in `r most_sig$CellType[1]`, with a z-score of `r round(most_sig$fold_change[1], digits = 2)`. In line with this, the associated adjusted p value is `r most_sig$p[1]`. Similarly strong results are represented through the rest of the table.
...
The above table shows that the highest number of significant enrichments is seen in `r top2_cells$CellType[1]`, closely followed by `r top2_cells$CellType[2]`.
...
For example, if we pull out any terms containing the word "`r params$keyword_1`", any containing the word "`r params$keyword_2`", and any containing the word "`r params$keyword_3`", we can search for any enrichments associated with these phenotypes and graph them according to cell types for comparison.

Below, some basic terms associated with "`r params$keyword_1`", "`r params$keyword_2`" and "`r params$keyword_3`" are used to pull phenotypes from the HPO. Then, the number of enrichments for each list of terms is graphed for each cell type in the dataset.  


* **`r params$keyword_1` associated terms:** `r paste(params$keyword_1_search_terms, collapse = ", ")`  
* **`r params$keyword_2` associated terms:** `r paste(params$keyword_2_search_terms, collapse = ", ")`  
* **`r params$keyword_3` associated terms:** `r paste(params$keyword_3_search_terms, collapse = ", ")`  

```{r, message = FALSE}
HeartTerms <- paste(paste0("(?i)", params$keyword_1_search_terms), collapse = "|")
CircTerms<- paste(paste0("(?i)", params$keyword_2_search_terms), collapse = "|")
GlucTerms <- paste(paste0("(?i)", params$keyword_3_search_terms), collapse = "|")
...
ggplot(combinedSumsMelt, aes(x = ct, y = value, fill = Group)) + geom_bar(stat = "identity", position = "dodge") + theme_cowplot() + 
  ...
  scale_y_continuous(expand = expansion(mult = c(0, .1))) +
  scale_y_continuous(breaks = pretty_breaks(), expand = expansion(mult = c(0, .1))) +
    theme(legend.position = "top", legend.key.size = unit(0.25, "cm"), legend.text = element_text(size = 10), legend.title = element_blank()) 
```  
...
```
In the above graph, it can be seen that for the "`r params$keyword_1`" variable, the highest proportion of significantly enriched phenotypes is associated with "`r names(HeartSums2[order(as.numeric(HeartSums2), decreasing = TRUE)][1])`", the sole cell population taken from heart tissue in Tabula Muris.

Furthermore, "`r params$keyword_2`"  has the highest proportion of phenotypes in `r names(CircSums2[order(as.numeric(CircSums2), decreasing = TRUE)][1])`, and "`r params$keyword_3`" and associated terms has by far the highest proportion of phenotypes associated with the `r names(GlucSums2[order(as.numeric(GlucSums2), decreasing = TRUE)][1])`, as may be reasonably expected.
...
Below, we will pull all terms that fall under "`r params$parent_term_example`", then use ggplot to visualise how many of these phenotypes have enrichments within the results dataframe, and what cells they are associated with.
...
```{r}
PlotDescendants(TERM = params$parent_term_example, PROPORTION = FALSE)
```


The above graph shows that all terms falling under "`r params$parent_term_example`", where enriched, are enriched solely in three types of pancreatic cells. Oddly, there are no enrichments associated with 'Pancreatic stellate cells', despite this cell type having the most enrichments in the entire set of results (see above). **(Write more general paragraph here?)**

#### Cell type enrichments associated with descendants of the "`r params$parent_term_example2`" phenotype

The same process as above can be replicated for any phenotype in the HPO. Below is the relevant plot for the "`r params$parent_term_example2`" phenotype.

```{r}
PlotDescendants(TERM = params$parent_term_example2, PROPORTION = FALSE)
```
As can be seen from the table, the vast majority of phenotypes under "`r params$parent_term_example2`", where enriched, are enriched in various immune cells. Note that there are three associated with hepatocytes, explored below. **(Not generalisable?)**
...
mydf <- GiveTable(TERM = params$parent_term_example2)

signif[which(signif$list %in% mydf$Term), ][1:10,]
```

Interestingly, in the top ten results of this list (out of 59), all three occurences of hepatocytes show up, the first two of which are extremely strong results with a fold change of almost 22 - 'Recurrent Neisserial infections' and 'Recurrent meningococcal disease'. **(again, not generalisable)**
...
Following on from where "`r params$parent_term_example`" related phenotypes were linked with expected cells types, a similar method can be used to determine whether certain branches of terms in the HPO are linked with expected or unexpected cell types.

Consider the following as an example:

1. Pull all HPO child terms falling under "`r params$expected_terms`".
2. Subset results down to significant results for a specific cell type that could be reasonably associated with those phenotypes (e.g. `r params$cell_of_interest` for "`r params$expected_terms`" related phenotypes).
...
HeartTerms <- params$expected_terms
...
all_cardiac_signif <- subset(all_results_merged, CellType == params$cell_of_interest & q < 0.05 & fold_change > 1 & sd_from_mean > 0)
...
* The total number of significant phenotype-cell associations for "`r params$cell_of_interest`" is `r nrow(all_cardiac_signif)`.

* The total number of significant phenotypes that are descendants of these terms, associated with "`r params$cell_of_interest`", is `r sum(unique(all_cardiac_signif$list) %in% df$Term)`.

* The total that are not is `r sum(!unique(all_cardiac_signif)$list %in% df$Term)`

As shown, roughly one third of sigificant enrichments associated with "`r params$cell_of_interest`" fall under the branches of heart terms we pulled from the HPO. **(Generalise?)**

Printing the first ten rows from each results set will give some idea of the types of phenotypes associated with "`r params$cell_of_interest`":
...
TrueHeart
patterns <- paste(paste0("(?i)", params$related_patterns), collapse = "|")
...
We can plot the results as a pie chart to show the proportions of different types of terms associated with "`r params$cell_of_interest`":

```{r}
HeartDF <- data.frame(matrix(ncol = 3, nrow = 1))
colnames(HeartDF) <- c(paste(params$expected_terms, "related terms"), paste("Related to:", paste(params$related_patterns, collapse=", ")), "Other terms")
rownames(HeartDF) <- c("value")

HeartDF[,paste(params$expected_terms, "related terms")] <- nrow(TrueHeart)
HeartDF[,paste("Related to:", paste(params$related_patterns, collapse=", "))] <- nrow(OtherHeart[which(OtherHeart$list %in% patternList)])
...
HeartBar <- ggplot(HeartMelt, aes(x = "", y = value, fill = variable)) +
  ...
  labs(x = paste(params$cell_of_interest, "enrichments"), y = "Number of enrichments")

HeartPie <- HeartBar +
  coord_polar("y", start = 0) +
  labs(title = paste("Categories of HPO terms enriched in",params$cell_of_interest,"(%)"), fill = "Term type") +
  ...
  labs(caption = paste("Total enrichments for",params$cell_of_interest,": ", sum(HeartMelt$value)))
...
Here, we can generate such a plot representing phenotypes strongly associated with `r params$enriched_cell`, but not at all significant in `r params$not_enriched_cell`, even when using uncorrected p values in the latter.
...
BCells <- params$not_enriched_cell_related_cells
TCells <- params$enriched_cell_related_cells
...
Furthermore, we can graph the total number of phenotype enrichments associated with `r params$enriched_cell`, in terms of those that are also associated with `r params$not_enriched_cell`, alongside those that are not.
...
colnames(TCellDF) <- c(paste(params$enriched_cell,"and",params$not_enriched_cell), paste(params$enriched_cell, "not", params$not_enriched_cell))
rownames(TCellDF) <- c("Value")

#TCellDF$Total[1] <- nrow(AllTCellEnrich)
TCellDF[,paste(params$enriched_cell, "not", params$not_enriched_cell)][1] <- nrow(OneSignifNotAnother(Yes = TCells, No = BCells))
TCellDF[,paste(params$enriched_cell,"and",params$not_enriched_cell)][1] <- nrow(BothSignif(First = TCells, Second = BCells))

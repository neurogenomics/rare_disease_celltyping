...
```{r overviewResults, fig.height = 20, fig.width = 20, fig.cap = "\\label{fig:overviewResults}\\textbf{Overview results} plots describing the number of significant results.", message = FALSE, warning = FALSE, eval= FALSE }
...
```{r overviewResults2, fig.height = 20, fig.width = 20, fig.cap = "\\label{fig:overviewResults}\\textbf{Overview results} plots describing the number of significant results.", message = FALSE, warning = FALSE }
...
# Test with ggnetwork
```{r testplot, fig.height = 20, fig.width = 20}
...
Fig2 = (  
  (plot_npc + theme(legend.position =c(0.7,0.3), axis.text.y = element_text(size = 12)))  +
  (main_branch_plt + theme(axis.text.y = element_text(size = 13))) +
          ggnetPlt + # try increase height_px (orig 300,1200) 
        ( facet_branch_plt + theme(axis.title.y =element_text(margin = margin(l = -15, b = -10))) ) + 
        plot_layout(design = layout) + plot_annotation(tag_levels = LETTERS) )
    ...
    color_other_phenotypes = 4, blank_x_axis = FALSE) 
{
    ...
    prop_plot <- ggplot(proportional_results, aes(x = minus_log_p, 
      ...
      scale_y_continuous(limits = c(0, 100)) + cowplot::theme_cowplot() + 
        ...
        legend.text = element_text(size = 6.5), title = element_text(size = 9))
      if (! blank_x_axis){
...
for (i in seq(length(plot_branches))) {
  cur_plot <- proportion_of_expected_enrichments_plot_2(all_results_merged,
   ...
   color_pal =  c("#619CFF","#F8766D","#00BA38","gray"),
...
```{r hpopatterns, fig.height = 12, fig.width = 15, fig.cap = "\\label{fig:hpopatterns}\\textbf{ontology level plots} Plots describing relationships between expected cell types, significance, and ontology level.", message = FALSE, warning = FALSE }
...
```{r infections, fig.height = 20, fig.width = 20, fig.cap = "\\label{fig:infections}\\textbf{Recurrent infections} Cells associated with recurrent infections.", message = FALSE, warning = FALSE , echo=FALSE}

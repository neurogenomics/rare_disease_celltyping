...
gen_results = function (CTD_file = "ctd_l1l2_nz.rda",
  ...
  if(!file.exists("data/MRK_List2.rpt")){
    utils::download.file("http://www.informatics.jax.org/downloads/reports/MRK_List2.rpt", destfile="data/MRK_List2.rpt")
  }
...
print("debug1")
  ...
  ctd[[1]]$specificity <- EWCE::fix_bad_mgi_symbols(data.frame(ctd[[1]]$specificity), mrk_file_path="data/MRK_List2.rpt")

print("debug2")

  ctd[[1]]$mean_exp <- EWCE::fix_bad_mgi_symbols(data.frame(ctd[[1]]$mean_exp), mrk_file_path="data/MRK_List2.rpt")

  genedata = utils::read.delim(phenotype_to_genes_txt_file, skip = 1, header = FALSE)  #HPO annotation
  ...
  RunEWCE = function(phenotype){
    ...
    human.hits = unique(geneList)

    #EWCE only works using lists of 3 or more genes.
    if(length(human.hits) > 3){
      full_results = EWCE::bootstrap_enrichment_test(sct_data = ctd, hits = human.hits, bg = human.bg, reps = reps, annotLevel = level,sctSpecies="human",genelistSpecies="human")
  ...
  all_results = parallel::mclapply(desiredPhenotypes, FUN = EWCEWrapper, mc.cores = mc.cores)
  ...
  all_results_merged$q = stats::p.adjust(all_results_merged$p, method = p_adj_method) #Went with BH for now

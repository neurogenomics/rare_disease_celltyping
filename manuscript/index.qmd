...
```{r fig-ontology-lvl, fig.height=7, fig.width=14}
...
(
  ...
  ggplot2::geom_segment(
    ggplot2::aes(x=1, xend = 1 , y=0, yend = 1), 
    size=2,color="grey50",
    arrow = ggplot2::arrow(ends="both", type="closed",
                           length = ggplot2::unit(0.6,"cm"))) +
  ggplot2::geom_label(ggplot2::aes(x=1, y=.8, label="Broad\nphenotypes"),
                     size=4, alpha=.8) +
  ggplot2::geom_label(ggplot2::aes(x=1, y=.2, label="Specific\nphenotypes"),
                     size=4, alpha=.8) +
  ggplot2::theme_void()
...
```{r fig-congenital, fig.height=5, fig.width=7}
...
Hypotonia (*HP:0001252*) is a very broad phenotype containing `r length(unique(hypotonia_results$hpo_id))-1` subterms (e.g. "Generalised neonatal hypotonia") and is associated with `r length(unique(hypotonia_results$disease_id))` unique diseases in the HPO gene annotations. Together, these hypotonia phenotypes were significantly associated with `r length(unique(hypotonia_results[q<0.05]$cl_name))`/`r length(unique(results$cl_name))` (`r format(length(unique(hypotonia_results[q<0.05]$cl_name))/length(unique(results$cl_name))*100, digits=4)`%) unique CL-aligned cell types. This reflects the highly variable set of disease etiologies that can cause this broad-level phenotype. Across all diseases, hypotonia phenotypes tended to be most consistently severe (lower mean age of death score) when associated with the cell type `r plot_differential_outcomes_heatmap_out$data_agg[1,]$celltype_symptom`. While other cell types were associated with lower mean age of death scores (e.g. `r paste(min_ageofdeath_score_celltypes[seq(2)],collapse=", ")`), the severity of the outcomes were more variable.
...
::: {#fig-therapy-examples .content-visible unless-format="html" height=80%}
![](img/fig-therapy-examples-merged.png)
...
::: {.content-hidden unless-format="html"}
```{r fig-therapy-examples, fig.height=8, fig.width=12, results='asis'}
...
Consider the following hypothetical scenario. A clinician observes that a newborn patient has several phenotypes (`r paste(shQuote(names(phenotypes_diagnose)),collapse = ", ")`), none of which conclusively point to a single disease diagnosis. Under the strong suspicion that the phenotypes are genetic in origin, the clinician orders whole-genome sequencing (WGS) on the patient as well as the patient’s family. The clinician finds that the patient has a number of putative causal mutations, narrowing down the number of potential diseases from hundreds to just 10. Further narrowing down the possibilities at this stage can be extremely challenging even for expert clinical geneticists. However, additional knowledge of which tissues and cell types are primarily affected allow the clinician to make a series of testable hypotheses that they may begin to investigate. For example, two of the putative diseases are known to cause aberrant splicing events in a gene that is only expressed in adrenocortical cells ([Fig. @fig-diagnosis]), providing justification to order a needle biopsy of the adrenal gland. RNA sequencing is performed on the tissue biopsy and it is discovered that the patient does indeed have high expression of the dysfunctional transcript, confirming the disease diagnosis [@Lord2021-rf]. This opens new avenues for the patient to receive timely and effective treatments for their specific condition, which is important as their version of the disease tends to lead to death in early childhood if left untreated ([Fig. @fig-prognosis]). Fortunately, their diagnosis now qualifies them to participate in a clinical trial of a novel gene therapy with promising preliminary results. Furthermore, it is predicted that this patient would respond especially well to this treatment given that the mechanisms of action of the gene therapy primarily acts on adrenocortical cells ([Fig. @fig-therapy-examples]).
...
(skipping until [simona](https://github.com/jokergoo/simona/issues/8) is fixed)

```{r fig-animal-models, fig.height=12, fig.width=10, eval=FALSE}
...
upheno <- KGExplorer::get_ontology(name="upheno", remove_cyclic_paths=TRUE, remove_rings=TRUE)
...
::: {.content-visible unless-format="html"}
![Respiratory failure](img/fig-therapy-examples-supp/respiratory_failure.png) ![Amyotrophic lateral sclerosis](img/fig-therapy-examples-supp/als.png)

![Dementia](img/fig-therapy-examples-supp/dementia.png) ![Small vessel disease](img/fig-therapy-examples-supp/small_vessel_disease.png)

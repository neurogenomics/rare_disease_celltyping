...
- **Additional file 2** (`additional_file_2.xlsx`): _Supplementary tables (machine-readable form)._
  Multi-sheet Excel workbook containing the same supplementary tables (Table S1-S11) but in fully machine-readable form, so the underlying numeric data can be reused without OCR. Sheet names match the in-text citations exactly (`Table S1`, `Table S2`, ..., `Table S11`). Generated programmatically from the same R data objects that produce the rendered tables, via an R chunk at the end of the Supplementary Tables source (see `manuscript/index.qmd`, chunk `write_additional_file_2`).
...
log_marker <- function(msg) {
  writeLines(paste(format(Sys.time(), "%H:%M:%OS3"), msg),
             con = "/tmp/xlsx_chunk_marker.txt")
}
log_marker("chunk started")
.write_af2 <- function() {
  if (!requireNamespace("openxlsx", quietly = TRUE)) {
    log_marker("ERROR: openxlsx not installed")
  ...
  # Excel limits sheet names to 31 characters, so keep them short ("Table SX").
  sheets <- list(
  ...
  log_marker(paste("assembled", length(sheets), "sheets"))
  # here::here() resolves to the manuscript/ dir (where .here lives), so we
  # only need _manuscript/ as a relative subpath.
  path <- here::here("_manuscript", "additional_file_2.xlsx")
  openxlsx::write.xlsx(sheets, file = path, asTable = FALSE, overwrite = TRUE)
  log_marker(paste("OK wrote", path))
}
tryCatch(.write_af2(),
         error = function(e) log_marker(paste("ERROR:", conditionMessage(e))))
```

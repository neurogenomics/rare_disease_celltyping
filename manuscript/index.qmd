...
In the HPO, a phenotype can be associated with multiple diseases via different subsets of genes ([Fig. @fig-diagram]).
...
We therefore tested for the consistency of our results across the two single-cell reference datasets (Descartes Human vs. Human Cell Landscape) for the subset of overlapping cell types [Fig. @fig-ctd-correlation].
...
All inter- and intra-dataset correlation tests were confirmed with permutation testing (1,000 permutations, @tbl-ctd_validation_permtests).
...
The top `r formals(MSTExplorer::run_congenital_enrichment)$top_n_hpo` foetal-biased phenotypes revealed were enriched for the HPO branches `r shQuote(run_congenital_enrichment_out$hpo_enrich_top$name[1])` ($p=$`r run_congenital_enrichment_out$hpo_enrich_top$p_value[1]`) and `r shQuote(run_congenital_enrichment_out$hpo_enrich_top$name[2])` ($p=$`r run_congenital_enrichment_out$hpo_enrich_top$p_value[2]`), which included specific phenotypes such as 'Abnormal morphology of the nasal alae' (see @tbl-hpo_enrich, and @tbl-fetal_examples).
...
To focus on clinically relevant phenotypes and reduce overplotting, we limited selection to those with GPT severity scores above `r severity_score_gpt_threshold` ([Fig. @fig-therapy-examples2]).
...
We identified alternatively activated macrophages as the cell type most associated with 'GM2-ganglioside accumulation' ([Fig. @fig-therapy-examples2]).
...
Additional examples of therapeutics targets include; cardiac muscle and endothelial cells in phenotypes associated with respiratory failure ([Fig. @fig-therapy-examples-supp-rf]), microglia in frontal lobe dementia ([Fig. @fig-therapy-examples-supp-dementia]), chondrocytes in lethal skeletal dyplasia ([Fig. @fig-therapy-examples-supp-lsd]), endothelial cells in small vessel disease ([Fig. @fig-therapy-examples-supp-svd]), oligodendrocytes and neurons in Parkinson's disease ([Fig. @fig-therapy-examples-supp-pd]).
and multiple gastrointestinal and immune cell types in Alzheimer's disease ([Fig. @fig-therapy-examples-supp-ad]).

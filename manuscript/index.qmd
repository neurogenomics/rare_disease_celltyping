...
plot_congenital_annotations_out_stats <- data.table::data.table(
  ...
  "$",
  "p=",formatter(p.value),",",
  "\\chi^2_{Pearson}=",formatter(statistic),",",
  "\\hat{V}_{Cramer}=",formatter(estimate),
  "$"
)]
```

To further verify the biological relevance of our results, we examined the association of foetal cell types with phenotypes annotated as congenital in onset. As expected, the frequency of congenital onset with each phenotype (as determined by GPT-4 annotations) was strongly predictive with the proportion of significantly associated foetal cell types in our results ($p=$`r plot_congenital_annotations_out_stats$p.value`, $\chi^2_{Pearson}=$`r plot_congenital_annotations_out_stats$statistic`, $\hat{V}_{Cramer}=$`r plot_congenital_annotations_out_stats$estimate`). Furthermore, increasing congenital frequency annotation (on an ordinal scale) corresponded to an increase in the proportion of foetal cell types: `r paste(plot_congenital_annotations_out_data[fetal_celltype==TRUE]$summary,collapse=", ")`. This is consistent with the expected role of foetal cell types in development and the aetiology of congenital disorders.
...
pheno_map_targets_severe[,summary:=paste0(
  ...
  " ($SIM_{o,g}=",round(phenotype_genotype_score,options()$digits),"$)"
  )] 
```
...
::: {.content-hidden unless-format="html"}
![](equations/eq1.png){#eq-evidence-scores height="300px"}
...
::: {.content-hidden unless-format="html"}
![](equations/eq3.png){#eq-ctd-specificity height="300px"}
...
::: {.content-hidden unless-format="html"}
![](equations/eq4.png){#eq-symptoms height="300px"} 
...
::: {.content-hidden unless-format="html"}
![](equations/eq5.png){#eq-gpt height="300px"}
...
::: 
![](img/fig-diagram.png){#fig-diagram}
...
::: {.content-visible unless-format="html"}
![Respiratory failure](img/fig-therapy-examples-supp/respiratory_failure.png){#fig-therapy-examples-supp}

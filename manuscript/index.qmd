...
Associations were validated against orthogonal biomedical knowledge graphs and prioritised by clinical severity and gene-therapy compatibility.
...
Source code: <https://github.com/neurogenomics/rare_disease_celltyping>.
...
Rare diseases (RDs) are individually uncommon but collectively this class of over 10,000 conditions affects 300–400 million people worldwide (1 in 10–20 individuals) [@Ferreira2019-jp; @Zhu2020-vo], 75\% of RD patients are children, with a 30\% rate of mortality by age five [@noauthor_undated-kp].
...
For association tests with exactly matching Cell Ontology ID across the two references, we tested for a relationship between the effect sizes (each GLM model's $R^2$ estimates) generated with each of the references using Pearson correlation (`stats::cor.test(method="pearson")`).
...
HPO branches varied significantly in the proportion of their significant assocations with foetal cell types ($\hat{V}_{Cramer}$=`r plot_congenital_annotations_branch_out$data_stats$summary_data$estimate`, $p<$`r plot_congenital_annotations_branch_out$data_stats$summary_data$p.value`, [Fig. @fig-congenital]b).
...
Each dataset and software artefact below is also listed in the Reference list as a full citation.

**Datasets:**

- Human Phenotype Ontology, release 2024-02-08 [@hpo_2024_release]: <https://hpo.jax.org>
- Gene Curation Coalition (GenCC) disease-gene curation database [@gencc_data]: <https://thegencc.org/>
- Descartes Human single-cell transcriptomic atlas [@descartes_human_data]: <https://cellxgene.cziscience.com/collections/c114c20f-1ef4-49a5-9c2e-d965787fb90c>
- Human Cell Landscape single-cell transcriptomic atlas [@human_cell_landscape_data]: <https://cellxgene.cziscience.com/collections/38833785-fac5-48fd-944a-0f62a4c23ed1>
- Processed Cell Type Datasets (*ctd_DescartesHuman.rds*, *ctd_HumanCellLandscape.rds*) and gene-by-phenotype association matrix (*hpo_matrix.rds*) and full phenotype-cell type results (*phenomix_results.tsv.gz*) [@ctd_data_archive]: <https://github.com/neurogenomics/MSTExplorer/releases>
- GPT-4 phenotype severity annotations [@gpt_severity_annotations]: <https://github.com/neurogenomics/rare_disease_celltyping/releases/download/latest/gpt_check_annot.csv.gz>
...
- Complement system gene list (HGNC group 492) [@hgnc_complement]: <https://www.genenames.org/data/genegroup/#!/group/492>
- Therapeutic Target Database (TTD) [@ttd_data]: <http://db.idrblab.net/ttd/>
...
- Cell Ontology, release 2023-09-21 [@cell_ontology_release]: <https://obophenotype.github.io/cell-ontology/>
- Monarch Knowledge Graph [@monarch_kg_data]: <https://monarchinitiative.org/>

**Source code (all under open-source licenses):**

- `KGExplorer` R package [@kgexplorer_pkg]: <https://github.com/neurogenomics/KGExplorer>
- `HPOExplorer` R package [@hpoexplorer_pkg]: <https://github.com/neurogenomics/HPOExplorer>
- `MSTExplorer` R package [@mstexplorer_pkg]: <https://github.com/neurogenomics/MSTExplorer>
- Manuscript analyses repository [@rdc_analyses_repo]: <https://github.com/neurogenomics/rare_disease_celltyping>
- Rare Disease Celltyping Portal source [@rdc_portal_code]: <https://github.com/neurogenomics/Rare-Disease-Web-Portal>
...
- **Additional file 1** (`additional_file_1.pdf`): _Supplementary figures and tables (display form)._
  PDF containing all supplementary figures (Fig. S1-S18) and supplementary tables (Table S1-S11) referenced in the main text, each with its full legend. Figure titles are formatted as "Fig S1", "Fig S2", ... and table titles as "Table S1:", "Table S2:", ..., matching the in-text citations of the form "Additional file 1: Fig. S1" and "Additional file 1: Table S1". Produced from the rendered manuscript PDF by extracting the Supplementary Materials section (`code/split_pdf.sh`).

- **Additional file 2** (`additional_file_2.xlsx`): _Supplementary tables (machine-readable form)._
  Multi-sheet Excel workbook containing the same supplementary tables (Table S1-S11) but in fully machine-readable form, so the underlying numeric data can be reused without OCR. Each sheet name encodes the table number (e.g., "Table S1 - HPO ontology mappings"). Generated programmatically from the same R data objects that produce the rendered tables, via an R chunk at the end of the Supplementary Tables source (see `manuscript/index.qmd`, chunk `write_additional_file_2`).
...
```{r write_additional_file_2, message=FALSE, warning=FALSE}
...
# (Table S1, Table S2, ...). Each sheet name corresponds to the table's
# rendered title in the PDF.
if (requireNamespace("openxlsx", quietly = TRUE)) {
  ...
  additional_file_2_sheets <- list(
    "Table S1 - HPO ontology mappings" = as.data.frame(maps_agg),
    "Table S2 - Enrichment summary" = data.frame(
      res_summ$tmerged[, -"ctd"],
      row.names = res_summ$tmerged$ctd,
      check.names = FALSE
    ) |> t() |> as.data.frame(),
    "Table S3 - Filtering steps" = as.data.frame(tbl_filters),
    "Table S4 - HPO-CL branch mappings" = as.data.frame(overrep_df),
    "Table S5 - GenCC encodings" = as.data.frame(gencc_tbl),
    "Table S6 - On-target cell types" = as.data.frame(ct_dt),
    "Table S7 - Ontology permutation tests" = as.data.frame(res_permtests),
    "Table S8 - CTD validation permutation" = as.data.frame(ctd_validation_permtest),
    "Table S9 - Foetal-biased phenotypes" = as.data.frame(foetal_biased_tbl),
    "Table S10 - Fetal phenotype examples" = as.data.frame(fetal_examples_tbl),
    "Table S11 - TTD hypergeometric" = ttd_hypergeo
  )
  additional_file_2_path <- here::here("manuscript", "_manuscript", "additional_file_2.xlsx")
  openxlsx::write.xlsx(
    additional_file_2_sheets,
    file = additional_file_2_path,
    asTable = FALSE,
    overwrite = TRUE
  )
  message("Wrote ", additional_file_2_path, " with ", length(additional_file_2_sheets), " sheets.")
} else {
  message("openxlsx not installed; skipping Additional file 2 generation.")
}
```

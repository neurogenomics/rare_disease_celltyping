...
associations between 201 cell types and 9,563/11,015 (86.6\%) unique
...
847,077 (2.35\%) tests across 77/ 77 (100\%) cell types and 7,330/11,047
...
(1.96\%) tests across 124/124 (100\%) cell types and 9,038/11,047
(81.8\%) phenotypes showed significant phenotype-cell type associations
...
(\(N_{p}\)) Abnormality of the cardiovascular system: 5/95 types of
`cardiocyte' were overrepresented (\(N_{p}\)=673). Abnormality of the
endocrine system: 3/84 types of `endocrine cell' were overrepresented
(\(N_{p}\)=291). Abnormality of the eye: 5/97 types of `photoreceptor
cell/retinal cell' were overrepresented (\(N_{p}\)=721). Abnormality of
the immune system: 14/82 types of `leukocyte' were overrepresented
(\(N_{p}\)=253). Abnormality of the musculoskeletal system: 4/99 types
of `cell of skeletal muscle/chondrocyte' were overrepresented
(\(N_{p}\)=2153). Abnormality of the nervous system: 17/98 types of
`neural cell' were overrepresented (\(N_{p}\)=1645). Abnormality of the
respiratory system: 3/89 types of `respiratory epithelial
cell/epithelial cell of lung' were overrepresented (\(N_{p}\)=291)..
...
(e.g.~`monocyte' vs.~`classical monocyte'), we captured 78\% of the MKG
phenotype-cell associations. Recall reached a maximum of 90.2\% at a
...
annotations, the recall rate of GPT-4 annotations was 91.43\%
(min=70.31\%, max=100\%, SD=11.76) with a mean consistency score of
...
\subsection{Diagnosis via cell type-specific disease
...
\subsection{Prognosis via cell type-mediated differential
...
averaged (Table~\ref{tbl-death}) to produce mean Age of Death scores for
...
5244 phenotypes across 4805 diseases in 201 cell types and 3141 genes
...
annotations per phenotype in the HPO overall (median=7, mean=62.16,
...
(837 phenotypes), followed by stromal cell (627 phenotypes), stromal
cell (627 phenotypes), neuron (474 phenotypes), chondrocyte (382
phenotypes), and endothelial cell (360 phenotypes). Grouped by
...
phenotypes, 1135 genes), `Abnormality of head or neck' (544 phenotypes,
981 genes), `Abnormality of the genitourinary system' (442 phenotypes,
693 genes), and `Abnormality of the eye' (377 phenotypes, 549 genes).
...
2024-04-15)\textsuperscript{52}. Overall, we prioritised 79\% of all
...
non-failed gene therapy targets (\(p=0.007\)). Importantly, we did not
...
therapeutics (\(p=3e-195\)). This suggests that our multi-scale
...
\includegraphics{img/fig-therapy-examples-merged.png}
...
disease'. Only phenotypes with a GPT severity score greater than 15 were
...
gangliosides accumulate within macrophages in TSD\textsuperscript{53},
as well as experimental evidence in rodent
models\textsuperscript{54.,55,56}. Our results not only corroborate
...
intervention\textsuperscript{57}, especially when genetic sequencing is
not available or variants of unknown significance are found within
\emph{HEXA}\textsuperscript{58}.
...
associated with different cell types via different phenotypes. For
example, AD 3 and AD 4 are primarily associated with cells of the
digestive system (`enterocyte', `gastric goblet cell') and are implied
to be responsible for the phenotypes `Senile plaques', `Alzheimer
disease', `Parietal hypometabolism in FDG PET'. Meanwhile, AD 2 is
primarily associated with immune cells (`alternatively activated
macrophage') and is implied to be responsible for the phenotypes
`Neurofibrillary tangles', `Long-tract signs'. This suggests that
different forms of AD may be driven by different cell types and
phenotypes, which may help to explain its variability in onset and
...
phenotypes. Amongst the 5244 phenotype within our prioritised therapy
...
diagnosis\textsuperscript{60}. This opens new avenues for the patient to
...
several years\textsuperscript{61--65} and proven remarkable clinical
success in an increasing number of clinical
applications\textsuperscript{66--69}. The U.S. Food and Drug
...
technologies\textsuperscript{70} with the aim of bringing dozens more
...
with a median of 8.3 years)\textsuperscript{71}. While these
...
root of an individual's conditions\textsuperscript{62,72}. A cell
...
antigens (e.g viral vectors)\textsuperscript{73}. This has the
...
costly\textsuperscript{9,10,61,64}. Here, we demonstrate the utility of
...
disease (e.g.~disease-associated microglia\textsuperscript{74,75}).
...
expert-curated GenCC annotations\textsuperscript{76,77} to weight the
...
cell types/subtypes\textsuperscript{78}.
...
EMBL-EBI Ontology Lookup Service\textsuperscript{79} and imported into R
...
(GenCC)\textsuperscript{76,77}, which (as of 2024-04-13) 21796 evidence
...
Benjamini-Hochberg false discovery rate\textsuperscript{80} (denoted as
...
correction\textsuperscript{80} (denoted as \(FDR _{b,c}\)), we annotated
...
many aspects of disease biology\textsuperscript{81}. This currently
...
assessing phenotype severity\textsuperscript{82}. Responses for each
...
severity from benign to debilitating to fatal\textsuperscript{83}. In
...
(Table~\ref{tbl-death}). To determine whether cell type identity
...
therapy from the Therapeutic Target Database (TTD; release 2024-04-15)
...
species from the MKG\textsuperscript{81}. Briefly, the ontological
...
phenotypes significant (\%) & 66.4 & 81.8 & 86.6 \\
...
(skipping until
\href{https://github.com/jokergoo/simona/issues/8}{simona} is fixed)

...
checked whether each cell type was overrepresented (at
\(FDR_{bc}<0.05\)) within its respective on-target HPO branch, where the
number of phenotypes within that branch. Indeed, we found that all 7 HPO
branches were disproportionately associated with on-target cell types
from their respective organ systems.
...
types that were overrpresented in the on-target HPO branch and the total
number of cell types in that branch. A disaggregated version of this
table with all descendant cell type names is availabe in
...
datasets (\(FDR_{pc}<0.05\)), we found that correlation of the
...
\begin{verbatim}
[1] "C5"   "C6"   "C8B"  "CFB"  "CFI"  "MBL2" "C7"  
\end{verbatim}

\begin{verbatim}
[1] "AFP_ALB_positive_cells"           "CCL19_CCL21_positive_cells"      
[3] "Adult_hESC"                       "Fetus_Stratified_epithelial_cell"
\end{verbatim}
...
were also associated with stromal cells
(FDR=\ensuremath{4.638\times 10^{-6}}, \(\beta\)=0.079), stratified
epithelial cells (FDR=\ensuremath{1.696\times 10^{-23}},
\(\beta\)=0.155), and embryonic stem cells
(FDR=\ensuremath{5.418\times 10^{-5}}, \(\beta\)=0.074). RNI is a
phenotype of 7 different diseases (`C5 deficiency', `C6 deficiency', `C7
deficiency', `Complement component 8 deficiency, type II', `Complement
factor B deficiency', `Complement factor I deficiency', `Mannose-Binding
lectin deficiency').
...
Severity annotations were gathered from GPT-4 for 16982/18082 (93.917\%)
HPO phenotypes in our companion study\textsuperscript{32}. Benchmarking
...
96.351\% (min=88.506\%, max=100\%, SD=4.541) with a mean consistency
score of 91.211\% (min=80.958\%, max=97.478\%, SD=5.739) for phenotypes
whose annotation were collected more than once. This clearly
demonstrates the ability of GPT-4 to accurately annotate phenotypes.
This allowed us to begin using these annotations to compute
...
severity score of 46.667, followed by `Anencephaly' (\emph{HP:0002323})
...
targets (mean=3.261, min=1, max=10). Relative to the number of genes
annotations per phenotype in the HPO overall (median=7, mean=61.951,
...
2024-07-02)\textsuperscript{51}. Overall, we prioritised 81.203\% of all
...
recapitulated 39.559\% of the non-failed therapeutic targets and 0\% of
the terminated/withdrawn therapeutic targets (n=1255). Here we found
that our prioritised targets were significantly enriched for non-failed
...
we recovered 81.203\% of all active gene therapies and 0\% of failed
gene therapies (Fig.~\ref{fig-therapy-validate},
...
therapy from the Therapeutic Target Database (TTD; release 2024-07-02)

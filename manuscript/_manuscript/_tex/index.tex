...
patients are children, with a 30\% rate of mortality by age
...
Curation Coalition (GenCC)\textsuperscript{23}. Analogously, we created
...
cell types Fig.~\ref{fig-ctd-correlation}. In total there were 142,285
...
CL term. We found that the correlation between association effect sizes
across the two datasets was significant (\(r_{Pearson}\)=\(0.58\),
\(p\)=\(1.4 \times 10^{-95}\)). Within the subset of phenotype-cell type
associations that were significant in both single-cell datasets
(FDR\textless0.05), we found that degree of correlation between the
association effect sizes across datasets was even stronger
(\(r_{Pearson}=\)\(0.73\), \(p=\)\(1.4 \times 10^{-95}\)). The symmetric
replication rate, i.e.~the mean percentage of associations that were
significant in both datasets at FDR\textless0.05, was 39\%.
...
\(p<\)\(2.2 \times 10^{-308}\); Table~\ref{tbl-ontology_permtests}).
...
type associations} and Fig.~\ref{fig-diagram} for further explanation).
...
proteins\textsuperscript{33}. However, emerging evidence indicates that
foetal hepatoblasts also express detectable amounts of complement
genes\textsuperscript{35}, which we confirmed by querying the CellxGene
browser\textsuperscript{36} (see \textbf{Data Availability}). Upon
further inspection, we found that these were in fact circulating
...
algorithm\textsuperscript{38}. \textbf{b} Expression specificity
quantiles (1-40 scale) of each driver gene in each cell type (darker =
greater specificity). \textbf{c} GenCC-derived eevidence scores between
...
DALL-E\textsuperscript{39}. The adult silhouette is from phylopic.org
...
with the most disproportiate number of foetal cell type associations
were `Abnormality of limbs' (\(35\)\%), `Growth abnormality' (\(32\)\%),
and `Abnormality of the musculoskeletal system' (\(29\)\%). The most
adult-biased brances were `Abnormality of blood and blood-forming
...
as `Abnormal morphology of the nasal alae' (see
Table~\ref{tbl-hpo_enrich}, and Table~\ref{tbl-fetal_examples}).
Adult-biased phenotypes were instead enriched for the branches `Abnormal
elasticity of skin' (\(p=\)\(3.6 \times 10^{-7}\)) and `Abnormality of
the cardiovascular system' (\(p=\)\(4.7 \times 10^{-3}\)), with examples
like `Excessive wrinkled skin' and `Paroxysmal supraventricular
tachycardia'. These align with known developmental and age-related
processes, supporting our approach for linking phenotypes to causal cell
types.
...
trials\textsuperscript{40--42}. Furthermore, knowing which cell types to
...
2026-02-25)\textsuperscript{43}. Overall, we prioritised \(87\)\% (120
...
Table~\ref{tbl-ttd_hypergeo}). For these hypergeometric tests, the
...
cases\textsuperscript{44}. In other cases, a tissue (e.g.~liver) may be
...
macrophages\textsuperscript{45,46.,47,48}. Our results support
...
cell type (Fig.~\ref{fig-therapy-examples2}). This suggests Purkinje
cell loss is downstream of macrophage dysfunction, consistent with
microglial roles in neurodegeneration\textsuperscript{49--51}. Our
...
(Fig.~\ref{fig-therapy-examples-supp-rf}), microglia in frontal lobe
dementia (Fig.~\ref{fig-therapy-examples-supp-dementia}), chondrocytes
in lethal skeletal dyplasia (Fig.~\ref{fig-therapy-examples-supp-lsd}),
endothelial cells in small vessel disease
(Fig.~\ref{fig-therapy-examples-supp-svd}), oligodendrocytes and neurons
in Parkinson's disease (Fig.~\ref{fig-therapy-examples-supp-pd}). and
multiple gastrointestinal and immune cell types in Alzheimer's disease
...
etiology as their common, polygenic counterparts\textsuperscript{53}.
...
disease\textsuperscript{56}, and the consistent association between
macrophages and neurodegenerative diseases such as Alzheimer's and
Parkinson's\textsuperscript{54}. Additionally, we recover genome-wide
...
retinal pigment cells, Mueller cells), immune system (dendritic cells,
macrophages), and vascular system (endothelial
cells)\textsuperscript{64}.
...
delivery\textsuperscript{65--68} are accelerating. The FDA's new
program\textsuperscript{69} aims to expand gene/cell therapy approvals
in years rather than decades\textsuperscript{70}, but success depends on
...
molecular etiologies across many diseases\textsuperscript{71}.
...
the EMBL-EBI Ontology Lookup Service\textsuperscript{72} and imported
...
ratio\textsuperscript{75}.

Therefore we imported data from the Gene Curation Coalition
(GenCC)\textsuperscript{76,77}, which (as of 2025-11-30) 24,124 evidence
...
atlas using the R package \texttt{EWCE} (v1.11.3)\textsuperscript{78}.
...
enrichment-based methods like \texttt{EWCE}\textsuperscript{80} would
...
our MKG benchmark) with a comparable degree of precision (see related
...
different subsets of genes (Fig.~\ref{fig-diagram}). Here we define a
symptom as a phenotype as it presents within the context of the specific
disease. A symptom's gene set is therefore the subset of genes
...
assessing phenotype severity\textsuperscript{81}. Responses for each
...
therapy from the Therapeutic Target Database (TTD; release 2026-02-25)
...
species from the MKG\textsuperscript{25}. Briefly, the ontological
...
macrophages in TSD\textsuperscript{45}, as well as experimental evidence
in rodent models\textsuperscript{46.,47,48}. Our results not only
...
intervention\textsuperscript{82}, especially when genetic sequencing is
not available or variants of unknown significance are found within
\emph{HEXA}\textsuperscript{83}.
...
disease\textsuperscript{49--51}. While experimental and postmortem
observational studies have implicated microglia in spinocerebellar
atrophy previously\textsuperscript{49}, our results provide a
...
targets, 154 had viable animal models in at least on non-human species.
...
arranged using the Sugiyama algorithm\textsuperscript{38}.}
...
\begin{tabular}[t]{l|r|r|l|r|r|r|r|r|r|r|r|r|r}
\hline

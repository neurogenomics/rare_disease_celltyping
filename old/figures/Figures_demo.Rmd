...
```{r}
library(HPOExplorer)
phenos <- all_results_merged[all_results_merged$CellType == "Acinar cells" & all_results_merged$q < 0.05 & complete.cases(all_results_merged), ]
adjacency <- HPOExplorer::adjacency_matrix(unique(phenos$HPO_id), hpo)

# Maybe need these column names to be options in make_network_object
phenos$HPO_Id <- phenos$HPO_id
phenos$Phenotype <- phenos$list
phenos$hover <- phenos$Phenotype


AcinarCells_net <-HPOExplorer::make_network_object(phenos, adjacency, hpo, colour_column = "fold_change")
plt = HPOExplorer::ggnetwork_plot(AcinarCells_net)
plot(plt)
...
print(cowplot::plot_grid(plotlist=list(ontlvl_ncells_plt,ontlvl_fold_plt,ontlvl_ngenes_plt),align = "hv",nrow = 1))

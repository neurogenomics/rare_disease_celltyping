...
\newpage
<!-- # Abstract Alternate version -->
<!-- Rare diseases (RD),  despite the name, are responsible for a significant proportion of the global burden of disease. However, due to the low prevalence of individual RDs, they often have not been allocated the resources needed to understand their pathophysiologies and develop therapies. The ever increasing avalibility of genomic, molecular and biomedical datasets has made it possible to beguin  uncovering novel insights into rare diseases, their genetic associations and the cell types involved. To accomplish this, lists of risk genes associated with dieases phenotypes were collected from the Human Phenotype Ontology (HPO) database. The gene lists were used as inputs for expression weighted cell type enrichment (EWCE) analysis, which identified the enrichment of primary cell types associated with the HPO gene lists. This anlaysis identifed over 8000 significant cell-phenotype associations (q*<0.05), and reproduced many previously known cell-phenotype  relationships documented in the literature, providing credibility to the many novel findings. Further, a subset of the novel cell-phenotype associations were examined and found to have have plausible mechanistic explanations. We can be confident that within these results there are many previously unknown insights that will be valuable targets for future research into RDs. To enable these results to have their maximum impact, the rare disease EWCE interactive results portal was developed to make them easily accessible to the community (https://ovrhuman.github.io/ewce_website/). -->
...
```{r Figure1,  fig.width = 20,fig.height=17, fig.cap = "\\label{fig:Figure1}\\textbf{Work flow.} \\textbf{A} This shows a subset of the human phenotype ontology, a directed acyclic graph of phenotypes, many annotated with associated gene lists. We utilised these gene lists along with scRNA-seq data from Descartes human cell atlas. \\textbf{B} We calculated gene list-cell specificity scores and compaired them to 100,000 randomly sampled gene list specificity scores to determine the probability of enrichment (EWCE). \\textbf{C} A interactive results portal was then created to allow domain experts to explore the results most relevant to them. This helps to prevent important findings being lost in a sea of results.", message=FALSE, echo=FALSE, warning =FALSE, results = FALSE}
...
```{r Figure2, fig.height=20, fig.width = 20, fig.cap = "\\label{fig:Figure2}\\textbf{A} This dendrogram shows the clustering of cell types in from the scRNA-seq data. The cells on the x axis are ordered by this grouping. \\textbf{B} Shows how many significant phenotype enrichments were found in total for each cell type, where significant is defined as \\textit{q}<0.05 and fold change > 1. The colours represent the tissues of origin of the expression data. \\textbf{C} Shows the number of significant enrichments per cell from within three chosen branches of the HPO. This helps us to see if the analysis is finding enrichments in expected cell types, given the broad class of penotype. For example, the vast majority of enrichments in the nervous system branch are assocaited with cells related to the nervous system, whereas these cells have almost no enrichments in the other two branches. A hypergeometric test was used to show where there is a significnatly elevated number of enrichments (asterics, ****, *** **, and *, indicate that \\textit{q}<0.00001, 0.0001, 0.001, and 0.05).", message = FALSE, warning = FALSE, echo = FALSE }
...
```{r Figure3, fig.height = 12, fig.width = 15, fig.cap = "\\label{fig:Figure3} \\textbf{A, B, and C} display the relationship between significance threshold and the proportion of enrichments found in the expected HPO branch. As more stringent significance thresholds for enrichment are used, the proportion of enrichments from an expected HPO branch increases. This validates the method as it shows that many of the strong phenotype-cell associations are in agreement with our current understanding. \\textbf{D, E, and F} show effecs related to ongology level, which we define as the number of generations of terms below a term, with the most general \"phenotypic abnormality\" at the top (level 12), and the most specific phenotypes at level 0, with no sub-types below them. \\textbf{D} shows that the number of cell types associated with a phenotype goes down as we approach more specific phenotypes. \\textbf{E} shows that these low level terms also typically have a higher fold change in specific expression. \\textbf{F} low level terms also typicaly have shorter gene lists than more general phenotypes. These effects suggest that many of the resulst found at the leaf nodes of the HPO may be the most actionable and fruitful for further investigation.", message = FALSE, warning = FALSE , echo = FALSE}
...
```{r Figure4,fig.width=20,fig.height=20, fig.cap = '\\label{fig:Figure4} \\textbf{A} Gives the number of significant enrichemnts per cell for descendant phenotypes of recurrant infection. As expected, cells associated with the immune system account for large numbers of enrichemnts. Aditionally, there are some surprising enrichments, for example, there is one in hepatoblasts. \\textbf{B} This shows the results for child terms of recurrent bacterial infection. We can see a strong association with cilliated epithelial cells in mycobacterial infections. This is unsurprising, given that this is primarily a respiratory disease, and dysfunctional cillia leave people prone to respiratory infection \\textbf{C} steps one ontology level down to greater specificity and looks at child terms of recurrent gram-negative bacterial infection. We can see that the surprising enrichemnt in hepatoblasts is found in from recurrent neiserial infections. While this seemed surprising, it has been documented in the literature that deficiency of complement (synthsized in the liver), increases susceptiblity to niesserial infection. In both \\textbf{B} and \\textbf{C}, significance is indicated with asterics (*, **, ***, and **** represent textit{q} less than 0.05, 0.001, 0.0001, and 0.00001. respectively).', message=FALSE, echo=FALSE}
...
chrVector2sentence <- function (char_vector, oxford_comma = TRUE) {
  sentence = ""
  for (i in seq(1,length(char_vector))) {
    if (i > 1 & i != length(char_vector)) {
      sentence = paste0(sentence, ", ")
    } else if (i == length(char_vector) & oxford_comma){
      sentence = paste0(sentence, ", and ")
    } else if (i == length(char_vector)) {
      sentence = paste0(sentence, " and ")
    }
    sentence = paste0(sentence, char_vector[i])
  }
  return (sentence)
}
...
More than 8000 significant enrichments were found. These are spread across all HPO branches. High-level HPO terms, encompassing a broad range of traits, were significantly associated with expected cell types, the number of expected enrichments also increases as more stringent significance thresholds are used. These findings validate the method and give credibility to the more novel results.

The highly specific low ontology level terms, found at the leaf nodes of the HPO, tended to have stronger enrichments and fewer associated genes. This potentially makes them good targets for research. One of the roadblocks of gene therapy is the lack of understanding of the pleiotropic effects of many genes [@gene_therapy_2020]. Being able to isolate the problem down to a small set of genes in a very specific cell type goes a long way to alleviating this problem. This may also provide a useful metric, along with disease severity, for prioritising further research. 

Whilst many of the findings were expected, there are a number of enrichments in less obvious cell types. For example, a significant number of terms in the cardiovascular branch were enriched in hepatoblasts (*p*<0.0001). On closer inspection, these phenotypes were primarily ones that often involve damage to arteries caused by lipid deposition (`r chrVector2sentence(tolower(cardio_hepato$Phenotype))`). Given the large role that the liver plays in lipid metabolism, it is unsurprising that dysfunction of hepatocytes may be implicated in these cardiovascular diseases. 

<!-- Additionally, a brief literature search finds many recent studies exploring the link between hepatic cells and cardiovascular diseases, such as the paper by @xu_hepatocyte_2021 which shows that hepatocyte ATF3 is protective against atherosclerosis by regulating high-density lipoprotein metabolism. Furhter, @bell_hepatocyte_2018 showed that higher circulating hepatocyte growth factor is associated with elevated atherosclerosis progression measures.  -->

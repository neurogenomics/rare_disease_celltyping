...
load("all_results_merged_100000rep_260121.rda") #Load results
library(ggplot2)
library(stringr)
library(tidyverse)
library(cowplot)
...
hpo <- get_OBO("hp.obo", propagate_relationships = "is_a", extract_tags = "minimal") #Loading hpo data within ontologyIndex
...
CircTerms <- c("(?i)circulating", "circulate", "circulation")
GlucTerms <- c("(?i)gluc", "(?i)glucose")

HeartPhenos <- unique(genedata$Phenotype[str_detect(genedata$Phenotype, pattern = HeartTerms)])
CircPhenos <- unique(genedata$Phenotype[str_detect(genedata$Phenotype, pattern = CircTerms)])
GlucPhenos <- unique(genedata$Phenotype[str_detect(genedata$Phenotype, pattern = GlucTerms)])
...
1. Pull all HPO child terms falling under a set of similar branches (e.g. branches about the heart).

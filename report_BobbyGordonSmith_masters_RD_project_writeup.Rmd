...
source("source/phenotype_gene_search.R")
...
figure_count = 0; figures = c()
table_count = 0; tables = c()
...
figure_count = figure_count + 1; figures$ADHD_ancestors = figure_count;
namer = function(term){
...
plot_phenos_per_cell1 = function (all_results_merged, cell_mappings, fold = 1, q_val = 0.005, dataset = "Descartes") {
  ...
  explan_plt <- ggplot(phenos_per_cell, aes(n_phenos ,Cell)) +
...
#round(-log10(all_results_merged$p+0.000001),digits=1)
...
# pearsons = list(cor.test(proportional_results$minus_log_p[proportional_results$cur_branch != "Other"],proportional_results$prop[proportional_results$cur_branch != "Other"],method="pearson"))
# pearsons[[1]]$branch =cur_branch
# #correlation_results = append(correlation_results, pearsons)
# correlation_results[cur_branch] = pearsons

correlation_results2 = rbind(correlation_results2,proportional_results)
  ...
  prop_plot <- ggplot(proportional_results, aes(x= minus_log_p, y = prop, color = cur_branch)) +
    ...
    labs(color="", linetype="") +
    ylab(paste0('% Enrichments in "', target_cell,'"')) +
    xlab("-log10(p)") +
...
pearsons_expected = cor.test(correlation_results2$minus_log_p[correlation_results2$cur_branch != "Other"],correlation_results2$prop[correlation_results2$cur_branch != "Other"],method="pearson")

# Reporting the results in text like this:
# (Nervous system *r*$_`r correlation_results[[1]]$parameter`$=`r correlation_results[[1]]$estimate`, *p*=`r correlation_results[[1]]$p.value`; Cardiovascular system *r*$_`r correlation_results[[2]]$parameter`$=`r correlation_results[[2]]$estimate`, *p*=`r correlation_results[[2]]$p.value`; Immune system *r*$_`r correlation_results[[3]]$parameter`$=`r correlation_results[[3]]$estimate`, *p*=`r correlation_results[[3]]$p.value`) **Remove correlation results?**, or give results for all in one 
...
print(cowplot::plot_grid(plotlist=list(lvlplt1,lvlplt2),align = "hv",nrow = 1,labels = c("A","B")))

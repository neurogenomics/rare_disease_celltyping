...
gen_ctd = function (output_ctd_file_name = "ctd_l1l2_nz.rda",
                    alpha = 0.05 ,
                    FACS_URL = "https://ndownloader.figshare.com/files/10700143" ,
                    annot_URL = "https://ndownloader.figshare.com/files/13088129" ,
                    L1_classifications_xlsx_path = "CTD_code/tm_level1classifications_full.xlsx"   ) {
  ...
  if (!file.exists(file.path("FACS"))){ #getw(), <- ?
    download.file(FACS_URL, "FACS.zip", mode = "wb")
    facs = unzip("FACS.zip",  exdir=getwd()); #unlink("__MACOSX", recursive = TRUE)  to delete an extra file in the tm link
    }
  if (!file.exists("annotations_facs.csv")){
    download.file(annot_URL, "annotations_facs.csv")}

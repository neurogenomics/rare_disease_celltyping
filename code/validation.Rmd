...
res <- lapply(stats::setNames(ctd_names,ctd_names), function(x){
  readRDS(paste0("/Volumes/bms20/projects/neurogenomics-lab/live/Projects/rare_disease_ewce/pbs/",x,"/rare_disease_min_genes4.rds"))
}) #|> data.table::rbindlist(idcol="ctd")
# ctd_lvls <- res[,unique(annotLevel),keyby="ctd"]
```
...
### Count number of unique celltypes per dataset ###
RES[,length(unique(CellType)), by=c("ctd","stage")]
```
...
obj_list_matched <- scOntoMatch::ontoMultiMatch(obj_list = obj_list,
                                               ...
                                               anno_col = 'cell_type',
                                               onto_id_col = "cell_type_ontology_term_id")
report_matches(obj_list, col="cell_ontology_mapped")
```
...
RES2 <- RES|> 
  ...
  formula = hpo_id+stage+CellType_mapped ~ ctd,
  fun.aggregate = mean,
  drop = TRUE,
  value.var = c("p","q","fold_change"))
RES2 <- RES2[complete.cases(RES2)][,test_id:=.I]
### All results
message(length(unique(RES2$CellType_mapped))," comparable celltypes.")
...
message(length(unique(RES_SIG$CellType_mapped))," comparable celltypes (FDR<0.05).")
...
RES2 |>
  ggstatsplot::ggscatterstats(x="p_HumanCellLandscape", 
    ...
    geom_density_2d_filled(alpha=.85) 
...
RES_HCL <- res$HumanCellLandscape |>
  subset(stage!="HESC")|>
  data.table::dcast.data.table(
  formula = hpo_id+CellType_mapped ~ stage,
  fun.aggregate = mean,
  drop = TRUE,
  value.var = c("p","q","fold_change"))
RES_HCL <- RES_HCL[complete.cases(RES_HCL)][,test_id:=.I]
### All results
message(length(unique(RES_HCL$CellType_mapped))," comparable celltypes.")
...
After manual inspection it was found that 100% of known causal cell types (and/or their immediate precursors) were recapitulated by our MultiEWCE results. In many instances, our results further resolved the specific cell subtype(s) underlying the phenotype (e.g. "neuron" to "visceromotor neuron", or "muscle cell" to "ventricular cardiac muscle cell").

Sperm and other germ cells could not be assessed due to their absence in both transcriptomic reference datasets.
```{r}
RES <- data.table::rbindlist(res_mapped, idcol = "ctd")

kg <- data.table::fread("~/Downloads/monarch_kg_cells.csv")
kg <- kg[from %in% unique(RES$hpo_id)]
message(paste(

...
```{=tex}
\begin{equation*}
  \eqnmarkbox[purple]{n1}{M_{g,p}}
  =
  \frac{
    \eqnmarkbox[red]{n3a}{\sum_{d \in D}}
    \eqnmarkbox[blue]{n4a}{R(g,p,d)} 
    \times 
    \eqnmarkbox[magenta]{n5}{E(g,d)}  
  }{
    \eqnmarkbox[red]{n3b}{\sum_{d \in D}}
...
The gene expression specificity matrix construction can be mathematically described as the following:

$$
S_{g,c} = \frac{\sum_{i=1}^{|L|}F(g,i,c)/N_c}{\sum_{r=1}^{k}(\sum_{i=1}^{|L|}F(g,i,r)/N_r)}
$$
...
```{=tex}
\begin{equation*}
  \eqnmarkbox[purple]{s1}{S_{g,c}}
  =
  \frac{
    \eqnmarkbox[red]{s3a}{\sum_{i=1}^{|L|}}
    \eqnmarkbox[blue]{s4a}{F(g,i,c)}/ 
    \eqnmarkbox[magenta]{s5a}{N_c}  
  }{
   \eqnmarkbox[BlueGreen]{s6}{\sum_{r=1}^{k}}(
     \eqnmarkbox[red]{s3b}{\sum_{i=1}^{|L|}}
     \eqnmarkbox[blue]{s4b}{F(g,i,r)}/ 
    \eqnmarkbox[magenta]{s5b}{N_r}  
   ) 
  }
\end{equation*}
\annotate[yshift=1em]{left}{s1}{Gene-by-cell type specificity matrix} 
\annotate[yshift=2em]{left}{s3a,s3b}{Average gene expression by cell type}
\annotate[yshift=-4em]{below,left}{s4a,s4b}{Gene-by-cell expression matrix}
\annotate[yshift=1em]{right}{s5a}{Number of cell types}
\annotate{below,right}{s5b}{Number of rows}
\annotate{below,left}{s6}{Row sums}
```
\
...
\begin{equation*}
  ...
  =
  \begin{cases}
  \eqnmarkbox[BlueGreen]{f2}{r_{g,i}},
  \text{ }l_i = c\\0,
  \text{ }l_i \neq c 
  \end{cases}
\end{equation*}
\annotate[yshift=1em]{left}{f1}{Filtered gene-by-cell type specificity matrix} 
...
\begin{equation*}
  \frac{
     \eqnmarkbox[BlueGreen]{g1}{|G_{d \cap p \cap c} |}
    }{
       \eqnmarkbox[blue]{g2}{|G_{d,p}|}} 
  \geq \eqnmarkbox[purple]{g3}{.25} 
\end{equation*}
...
```{=tex}
\begin{equation*}
  \eqnmarkbox[purple]{nss}{NSS_p}
  =
  \frac{ 
    \eqnmarkbox[blue]{nss2}{\sum_{j=1}^{m}} 
    (
      \eqnmarkbox[magenta]{nss3}{F_{pj}}
      \times 
      \eqnmarkbox[ForestGreen]{nss4}{W_j}
    )
    }{
    \eqnmarkbox[red]{nss5}{\sum_{j=1}^{m}(\max\{F_j\} \times W_j)} 
  } \times 100
\end{equation*}
